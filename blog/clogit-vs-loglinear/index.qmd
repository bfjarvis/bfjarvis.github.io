---
title: "Log-Linear vs. Multinomial vs. Conditional Logit Models of Assortative Mating"
subtitle: "Discrete Choice Models of Assortative Mating, Part 2"
...
\theta_{fm} = \delta_{fm}.
$$
Under this parameterization, $\theta_{fm}$ and $\delta_{fm}$ would be equivalent right off the bat. But the group-specific intercepts from the two models, $\gamma_m$ and $\eta_m$, would differ by $\log(N_m)$. But a simple reshuffling shows a quick fix: 

$$
\eta_m = \gamma_m - \log N_m
$$

This means that specifying $-\log(N_m)$ as an offset when estimating the conditional logit model will lead the group-specific intercepts to match $\gamma_m$ directly. In other words, in the model:
...
saturated_parameters <-
  bind_rows(
    ...
    `clogit woman` = broom::tidy(clogit_mSa),
    `clogit man` = broom::tidy(clogit_mSb),
    .id = "model"
  ) |>
